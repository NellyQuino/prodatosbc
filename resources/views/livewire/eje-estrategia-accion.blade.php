<div>
    <div class="form-group">
        ...
        <select wire:model="selectedEje" class="form-control" required>
            ...
            @foreach($ejes as $eje)
            <option value="{{ $eje->id }}"  >{{ $eje->number }}.- {{ $eje->name }}</option>
            @endforeach
        </select>
    </div>
    @if (!is_null($selectedEje))
    <div class="form-group select">
        <label for="problematica" class="col-md-4 col-form-label" style="font-size:120%;font-family:inter;">Problemática</label>
        <select wire:model="selectedProblematica" class="form-control" required>
            <option  selected>-- Elige una Problemática --</option>
            @foreach($problematicas as $problematica)
            <option value="{{ $problematica->id }}">{{ $problematica->number }}.- {{ $problematica->name }}</option>
            @endforeach
        </select>
    </div>
    ...
    <div class="form-group select">
        ...
        <select wire:model="selectedEstrategia" class="form-control" required>
            ...
            <option value="{{ $estrategia->id }}">{{ $estrategia->number }}.- {{ $estrategia->name }}</option>
            @endforeach
        </select>
    </div>
    @endif
    ...
    <div class="form-group">
        ...
        <select wire:model="selectedAccion"  class="form-control @error('accion_id') is-invalid @enderror" name="accion_id" required> 
            ...
            <option value="{{ $accion->id }}">{{ $accion->number }}.- {{ $accion->name }}</option>
            @endforeach
        </select>
        @error('accion_id')
